...
var Index = function(start) {
	...
	function init()
	{
		...
		for(var i = 0; i < scriptsArray.length; i++){
			...
			c.addEventListener("mousemove", e => that.handler(e), false);
			c.addEventListener("mouseout", e => that.handler(e), false);
			c.addEventListener("mousedown", e => that.handler(e), false);
			c.addEventListener("mouseup", e => that.handler(e), false);
...
function Mirrormouse3(context){
	...
	{
		e = e == undefined ? event : e;
		const pointOnLayer = { x: e.clientX - e.layerX, y: e.clientY - e.layerY }
		switch(e.type)
		{
			case "mousemove":
				if(first){
					...
					mouseVector.final = pointOnLayer;
					draw();
				}
			break;
			case "mouseout":
